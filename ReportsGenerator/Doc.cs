...
internal static class Doc
{
    public static object Read(BackgroundWorker bw, string drawName)
    {
        ...
        var table = document.Tables.Cast<Table>().FirstOrDefault(t => t.Columns.Count == 23);
        ...
        for (var j = startLine; j <= totalLines; j++)
        {
            ...
            if (columns[3].ToUpper().Contains("СВОДНЫЕ ДАННЫЕ")) break;

            if (string.IsNullOrWhiteSpace(columns[1]) || columns.Length != 23) continue;
            ...
            if (thickness.Contains("полособульб")) thickness = thickness.Trim().Replace("полособульб ", "r");
            ...
            if (!string.IsNullOrWhiteSpace(thickness))
            {
                if (Strings.InStr(thickness, "x") > 0)
                {
                    ...
                    if (Strings.InStr(array6[0], ".") < 1) array6[0] = $"{array6[0]}.0";
                ...
                else if ((Strings.InStr(thickness, "r") > 0) | (Strings.InStr(thickness, "р") > 0))
                {
                    ...
                    if (!Information.IsNothing(dataRow)) thickness = dataRow[1].ToString();
                }
            }
